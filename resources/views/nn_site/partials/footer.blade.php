    ...
    <div class="footer-container">
        <div class="footer-main container">
            <div class="footer-left">
                ...
                <div class="socials">
                    @if ($siteSettings->facebook)
                        <a href="{{$siteSettings->facebook}}" target="_blank" rel="noopener"><img src="/assets/images/facebook 1.svg" alt="Facebook"></a>
                    @endif
                    @if ($siteSettings->twitter)
                        <a href="{{$siteSettings->twitter}}" target="_blank" rel="noopener"><img src="/assets/images/twitter-sign 1.svg" alt="Twitter"></a>
                    @endif
                    @if ($siteSettings->linkedin)
                        <a href="{{$siteSettings->linkedin}}" target="_blank" rel="noopener"><img src="/assets/images/linkedin 1.svg" alt="LinkedIn"></a>
                    @endif
                </div>
            </div>
            ...
            <div class="footer-right">
                ...
                <p>{{$lang->subscribeDescr}}</p>
                <div class="subscribe-div">
                    <form action="{{url('subscribe')}}" class="subscribe" method="post" autocomplete="off">
                        @csrf
                        <input id="emailsubscribe" type="email" required placeholder="{{$lang->yourEmail}}" size="55" name="email">
        ...
        footerSubscribeForm.submit(function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    footerSubscribeFormText.text('{{$lang->successfullySubscribed}}');
                ...
                error: function (request, status, error) {
                    ...
                    footerSubscribeFormText.css('display', 'block');
                    footerSubscribeFormText.text('{{$lang->emailAlreadyExists}}');
                }
            });
        });
    </script>
...
@if (isset($isHomePage) || isset($isBankCurrencies))
    ...
    <script>
        ...
        function fillExchange(act = '') {
            ...
            $.post("/get-currencies-data",
            {
                ...
                obj:'{{request()->route("obj")}}',
                active:'{{request()->route("active")}}',
            ...
            function getChart(code="", dateofch="") {
                ...
                $.post("{{url((getCurrentLocale() == 'ka' ? '' : getCurrentLocale() . '/') . 'get-currencies-data-chart-days')}}",
                {
                    _token: '{{csrf_token()}}',
                    dateofch: dateofch,
                    code:code
                ...
                function(data){
                    ...
                    for (var arrItem in arr) {
                        ...
                        if (arr[arrItem][0] >= arr[arrItem][1]) {
                            $('#line-image-' + arrItem).attr('src', '{{url('')}}/assets/images/chart-success-'+Math.floor(Math.random() * 2 + 1)+'.svg');
                        } else {
                            $('#line-image-' + arrItem).attr('src', '{{url('')}}/assets/images/chart-down-'+Math.floor(Math.random() * 2 + 1)+'.svg');
                        }
                    }
    ...
    function changeRates(obj, onch=0) {
                ...
                if(onch != 1){
                    ...
                    if('{{request()->route("date")}}' != '' && onch != 'date'){
                    
                        $("#dateexchangerate").val('{{request()->route("date")}}');
                        $('#dateexchangerate-value').html('{{request()->route("date")}}');
            ...
            datet = ($("#dateexchangerate").val() =='')?'{{date("Y-m-d",time())}}':$("#dateexchangerate").val();
            var link = '';
            var pushUrl = "{{getCurrentLocale() == 'ka' ? '' : '/' .getCurrentLocale()}}/currencies/"+code+"-"+value+"/"+codesturul+"/"+datet;
            $('.link-share-button').each(function( index ) {
                ...
                $(this).attr('href', link+'{{url('')}}'+pushUrl);
            });
